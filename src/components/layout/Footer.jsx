...
export const Footer = () => {
   ...
   const adminItem = (() => {
      if (user?.role === "admin") {
         return { id: "admin", label: "Panel", Icon: LayoutDashboard, path: "/admin" };
      }

      if (user?.role === "hostelero") {
         return { id: "admin", label: "Panel", Icon: ChefHat, path: "/host/dashboard" };
      }
      ...
      <>
         ...
         <footer className="pointer-events-none fixed inset-x-0 bottom-0 z-50 pb-3">
            <div className="mx-auto flex w-full max-w-5xl justify-center px-4">
               <nav className="pointer-events-auto flex items-center gap-1 rounded-2xl border border-neutral-700/80 bg-neutral-900/88 p-1.5 shadow-[0_14px_34px_rgba(0,0,0,0.45)] backdrop-blur-xl" aria-label="Navegación principal">
                  {items.map((item) => {
                              ...
                              {item.id === "profile" && item.avatar ? (
                                 <img
                                    ...
                                    className={`h-6 w-6 rounded-full object-cover ${
                                       isActive ? "ring-2 ring-orange-500/90" : "ring-1 ring-neutral-600"
                                    }`}
                                 />
                              ) : (
                                 <ItemIcon
